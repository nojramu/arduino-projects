  ...
  const int LEDpins[] = {10,9,8,7,6,5,4}; // define the Arduino LED pins in use
  const int led1 = 2;
  const int led2 = 3;
  const int led3 = 11;
  int rows= 7; 
  ...
  int timer =333;
  int i;
  String top_str = "  BSCPE   ";
  String bottom_str = "    3B    ";
  const int ss = A0;
  int val;
  int cntr;
...
void Names(){
  Serial.println(top_str);
  Serial.println(bottom_str);
  ...
  void printLetterBottom(char ch)
  {
    // make sure the character is within the alphabet bounds (defined by the font.h file)
    // if it's not, make it a blank character
    if (ch < 32 || ch > 126){
  ...
  void loop(){
     ...
     for (int k=0; k<bottom_str.length(); k++){
       printLetterBottom(bottom_str.charAt(k));
       }
      digitalWrite(led1, HIGH);
      delay(4);
      digitalWrite(led1, LOW);
     for (int k=(top_str.length()-1);k>-1; k--){
       printLetterTop(top_str.charAt(k));
       }  
     digitalWrite(led1, HIGH);
     delay(4);
  }
