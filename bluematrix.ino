...
char msg[] =" BSCPE   "; //Change the text here.
int scrollspeed = 8;//Set the scroll speed ( lower=faster)
char blueval;
String convert;
int add;
int x;
...
//Columns
int clockPin1 = 4; //Arduino pin connected to Clock Pin 11 of 74HC595
int latchPin1 = 3; //Arduino pin connected to Latch Pin 12 of 74HC595
int dataPin1 = 2; //Arduino pin connected to Data Pin 14 of 74HC595
...
void Names(){
  ...
  if (Serial.available()){
    blueval = Serial.read();
    convert = convert + blueval;
    
    if(blueval == '*'){
      convert = convert.substring(0,convert.length()-1);
      convert = convert + "      ";
      convert.toCharArray(msg,convert.length()+1);
      convert = "";
    }
  }
}
